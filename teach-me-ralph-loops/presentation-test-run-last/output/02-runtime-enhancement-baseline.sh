...
#
# Verifies:
# 1. Deploy the SAME app as experiment 1 but WITHOUT build-time enhancement
# 2. Runtime class transformer (jboss.as.jpa.classtransformer=true) activates
# 3. Entity becomes enhanced at deployment time via the runtime enhancer
# 4. Lazy loading and persistence still work correctly
#
# This establishes the baseline that build-time enhancement is meant to replace.
...
for jar in "$HIBERNATE_CORE" "$JPA_API" "$JBOSS_LOGGING" "$TX_API" \
...
echo "Build-time enhancement: DISABLED"
echo "Runtime class transformer: ENABLED"
...
# persistence.xml with classtransformer=true (runtime enhancement)
...
<persistence xmlns="https://jakarta.ee/xml/ns/persistence"
    ...
    <persistence-unit name="testPU">
        ...
        <properties>
            ...
            <!-- Enable runtime class transformer so entities are enhanced at deploy time -->
...
echo ""
echo "--- Step 2: Compiling test application (no enhancement step) ---"
...
# ---- Step 3: Verify classes are NOT enhanced ----
echo ""
echo "--- Step 3: Verifying classes are NOT enhanced (pre-deploy) ---"
...
if echo "$JAVAP_OUTPUT" | grep -q "ManagedEntity\|PersistentAttributeInterceptable\|SelfDirtinessTracker\|EnhancementInfo\|\$\$_hibernate"; then
    echo ""
    echo "WARNING: Entity class appears to be enhanced BEFORE deployment -- unexpected"
    PRE_ENHANCED=true
else
    echo ""
    echo "VERIFIED: Entity class is plain (not enhanced) -- runtime enhancer should activate"
    PRE_ENHANCED=false
fi
...
# Enable TRACE logging for enhancement and JPA subsystem
...
# ---- Step 8: Check server log for runtime enhancement messages ----
echo ""
echo "--- Step 8: Checking server log for runtime enhancement activity ---"

SERVER_LOG="$EAP_HOME/standalone/log/server.log"

echo "Class transformer / enhancement log messages:"
grep -i "class.transformer\|classtransformer\|enhance\|bytecode" "$SERVER_LOG" | grep -v "^$" | head -30 || echo "(none found)"

echo ""
echo "JPA deployment messages:"
grep -i "persistence.unit\|transformer\|addTransformer\|register" "$SERVER_LOG" | head -20 || echo "(none found)"
...
if [ "$PRE_ENHANCED" = "true" ]; then
    echo "FAIL: Entity class was already enhanced before deployment"
    RESULT="FAIL"
else
    echo "OK: Entity class was plain (not enhanced) before deployment"
fi
...
if [ "$HTTP_CODE" = "200" ]; then
    ...
    if echo "$HTTP_BODY" | grep -q "RUNTIME_ENHANCED=true"; then
        echo "OK: ManagedTypeHelper confirms entity is runtime-enhanced"
    else
        echo "INFO: ManagedTypeHelper reports entity is NOT enhanced at runtime"
        echo "  (runtime enhancement may not set ManagedEntity marker in all cases)"
    fi
    ...
    if echo "$HTTP_BODY" | grep -q "HAS_ENHANCEMENT_INFO=false"; then
        echo "OK: No @EnhancementInfo annotation (expected -- runtime enhancement does not add it)"
    elif echo "$HTTP_BODY" | grep -q "HAS_ENHANCEMENT_INFO=true"; then
        echo "INFO: @EnhancementInfo annotation found (unexpected for runtime enhancement)"
    fi
