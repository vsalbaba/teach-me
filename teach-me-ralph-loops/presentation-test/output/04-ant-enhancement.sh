...
#
# Verifies:
# 1. A custom Ant task wrapping Hibernate's Enhancer SPI can enhance entity classes
# 2. Enhanced entities deploy correctly to EAP 8.2 with classtransformer=false
# 3. Enhancement is active at runtime (ManagedTypeHelper, PersistentAttributeInterceptable)
# 4. Application works correctly (entity persistence create/read)
#
# Note: Hibernate ORM 6.6.x does not ship an Ant EnhancementTask.
# We create a custom Ant task that uses the same Enhancer SPI as Experiment 1.
...
for jar in "$HIBERNATE_CORE" "$JPA_API" "$BYTE_BUDDY" "$JBOSS_LOGGING" "$TX_API" \
           "$SERVLET_API" "$EJB_API" "$INJECT_API" "$ANNOTATION_API" "$ANT_JAR"; do
    if [ ! -f "$jar" ]; then
...
echo "Using custom Ant task wrapping Hibernate Enhancer SPI"
...
# Custom Ant task wrapping Hibernate Enhancer SPI
...
# ---- Step 2: Compile the application and custom Ant task ----
echo ""
echo "--- Step 2: Compiling test application and custom Ant task ---"

BUILD_DIR="$WORK_DIR/classes"
TASK_DIR="$WORK_DIR/task-classes"
mkdir -p "$BUILD_DIR"
mkdir -p "$TASK_DIR"

COMPILE_CP="$JPA_API:$SERVLET_API:$TX_API:$ANNOTATION_API:$INJECT_API:$EJB_API"

# Compile the entity and servlet
javac -d "$BUILD_DIR" \
...
TASK_CP="$ANT_JAR:$HIBERNATE_CORE:$JPA_API:$BYTE_BUDDY:$JBOSS_LOGGING:$TX_API:$INJECT_API:$BUILD_DIR"

javac -d "$TASK_DIR" \
    -classpath "$TASK_CP" \
    -source 17 -target 17 \
    "$APP_DIR/src/main/java/com/test/ant/HibernateEnhancementTask.java"

echo "Custom Ant task compiled"

# ---- Step 3: Create Ant build.xml and run enhancement ----
echo ""
echo "--- Step 3: Running Ant-based bytecode enhancement ---"

ENHANCER_CP="$TASK_DIR:$HIBERNATE_CORE:$JPA_API:$BYTE_BUDDY:$JBOSS_LOGGING:$TX_API:$INJECT_API"

cat > "$WORK_DIR/build.xml" << EOF
<?xml version="1.0" encoding="UTF-8"?>
<project name="hibernate-enhancement" default="enhance" basedir=".">

    <path id="enhancer.classpath">
        <pathelement location="$TASK_DIR"/>
    ...
    <target name="enhance">
        <hibernate-enhance base="$BUILD_DIR"
...
ant -f "$WORK_DIR/build.xml" enhance
ANT_EXIT=$?

if [ "$ANT_EXIT" -ne 0 ]; then
    echo "ERROR: Ant enhancement failed with exit code $ANT_EXIT"
    exit 1
fi

echo "Ant enhancement completed successfully"
...
if echo "$JAVAP_OUTPUT" | grep -q "ManagedEntity\|PersistentAttributeInterceptable\|SelfDirtinessTracker\|EnhancementInfo\|\$\$_hibernate"; then
    echo ""
    echo "VERIFIED: Entity class has been bytecode-enhanced via Ant task"
    ENHANCED=true
else
    echo ""
...
if [ "$ENHANCED" != "true" ]; then
    echo "FAIL: Entity class was not enhanced by Ant task"
    RESULT="FAIL"
fi
